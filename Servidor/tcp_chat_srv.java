...
class tcp_chat_srv {
    ...
    public static String getNameHashCode() {
        int hash = 1;
        try {
            hash = hash * 2 + InetAddress.getLocalHost().getHostAddress().hashCode();
        } catch (UnknownHostException ex) {
            Logger.getLogger(startUDP.class.getName()).log(Level.SEVERE, null, ex);
        }
...
class startUDP implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
            ...
            System.out.println("Name "+data1.toString()+" L: "+res);
